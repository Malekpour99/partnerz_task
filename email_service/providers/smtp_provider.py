...
class SMTPProvider(EmailProvider):
    ...
    def _create_mime_message(self, email_message: EmailMessage) -> MIMEMultipart:
        ...
        # Attach HTML content
        html_part = MIMEText(
            email_message.html_content.encode("utf-8"), "html", "utf-8"
        )
        ...
        if email_message.attachment_path:
            self._add_attachment(
                mime_message,
                email_message.attachment_path,
                email_message.attachment_name,
            )

        return mime_message

    def _add_attachment(
        self, mime_message: MIMEMultipart, filepath: Path, filename: Optional[str]
    ) -> None:
        ...
        while retries <= self._max_retries:
            try:
                self._connection.sendmail(
                    message.sender, message.recipients, mime_message.as_string()
                )
                return
